# map of GitHub ID and Author ID from WoC
...
zcat /data/basemaps/gz/asQ.gz | perl -e 'while(<STDIN>){chop();m/(.*)<(.*)>/;$n=$1;$n=~tr/[A-Z]/[a-z]/;$n=~s/^\s+//;$n=~s/\s+$//;$n=~s/"//g;($f,@l)=split(/ /,$n); $ff{$f}++; $ll{$#l}++};while (($f, $n) = each %ff){print "f;$f;$n\n";}while (($f, $n) = each %ll){print "l;$f;$n\n";}' | gzip > N.freq
zcat /data/basemaps/gz/asQ.gz | perl -e 'while(<STDIN>){chop();m/(.*)<(.*)>/;$n=$1;$n=~tr/[A-Z]/[a-z]/;$n=~s/^\s+//;$n=~s/\s+$//;$n=~s/"//g;($f,@l)=split(/ /,$n); $ff{"$f;$ll{$#l}"}++;} while (($f, $n) = each %ff){print "$f;$n\n";}' | gzip > FL.freq

zcat /data/basemaps/gz/asQ.gz | \
  perl -e 'open A, "zcat FL.freq|";while(<A>){chop();($f,$n,$c)=split(/;/);$cnt{"$f;$l"}=$c;} while(<STDIN>){ chop();m/(.*)<(.*)>/;$n=$1;$n=~tr/[A-Z]/[a-z]/;$n=~s/^\s+//;$n=~s/\s+$//;$n=~s/"//g;($f,@l)=split(/ /,$n);print "$n;$_\n" if $#l > 0 && length ($f) > 3 && length($l[$#l]) > 3 && $cnt{"$f;$l[$#l]"} < 12;}' | \
	lsort 100G -t\; -k1,2 | gzip >  /data/basemaps/gz/N2aQ.gz.clean
...
zcat /data/basemaps/gz/N2aQ.gz.clean | \
  perl -e '$p="";while(<STDIN>){($c,@x)=split(/;/); if ($c ne $p && $p ne ""){ if (scalar(keys %tmp)>1){ for $i (keys %tmp){ print "$i"};} %tmp=(); }; $p=$c; $tmp{$_}++;}' | \
	perl -ane '@x=split(/;/);print if $x[0] =~ m/^.+\@[^\.]+\.[^\.]+/ && length($x[0]) > 8; ' | \
	perl $HOME/lookup/connectExportPreNoExclude.perl | gzip > N2aQ.p2p
...
k=12
for i in E2aQ N2aQ
do zcat $i.p2p | perl -ane 'chop();@x=split(/;/);print "$_\n" if $#x<'$k';'|gzip > $i.p2p.$k 
(zcat $i.p2p.$k |perl $HOME/lookup/connectExportSrt.perl $i;zcat $i.versions |  perl $HOME/lookup/connectPrune.perl |gzip > $i.versions1; zcat $i.versions1| ~/bin/connect | gzip > $i.clones; perl $HOME/lookup/connectImport.perl $i | gzip > $i.map.$k) 
done
...
#--------------------------------------------------------------------------------------
#use only user ids and only the ones that are not fake
...
#--------------------------------------------------------------------------------------
# Now use both Email and gh maps
i=egh
...
# Now use four
i=ghtNE2aQ
zcat {N2aQ,E2aQ,ght2aQ,a2ghQ}.p2p.$k | perl $HOME/lookup/connectExportSrt.perl $i
zcat $i.versions |  perl $HOME/lookup/connectPrune.perl |gzip > $i.versions1
zcat $i.versions1 | ~/bin/connect | gzip > $i.clones
perl $HOME/lookup/connectImport.perl $i | gzip > $i.map.$k
...
i=egh
k=12
zcat $i.map.12 | awk -F\; '{print $2";"$1}' | perl $HOME/lookup/connectExportPreNoExclude.perl | gzip > egh.map.12.p2p
zcat $i.map.12.p2p | perl -ane '$i++;chop (); @x=split(/;/); for $a (@x){ $a=~ m/([^<]*)<([^>]*)>/; $n=$1;$e=$2;$n=~s/\s*$//;print "$i;$n;$e;$a\n"};' | gzip > $i.forML.gz
